...
-- Calcula la facturación total por vendedor
-- y muestra los 10 mejores
SELECT
    concat(
...
-- Consulta 3: Vendedores con ingreso promedio
-- por debajo del promedio general
-- Identifica a los vendedores que necesitan
-- apoyo comparando su promedio individual
-- con el general
WITH seller_avg AS (
    SELECT
        e.employee_id,
...
overall_avg AS (
    SELECT avg(sa1.average_income) AS avg_income
    FROM seller_avg AS sa1
)
...
WHERE sa.average_income < oa.avg_income
ORDER BY sa.average_income;

-- Consulta 4: Ingresos por vendedor y día
-- Muestra cómo varían las ventas según el día
-- para cada vendedor
SELECT
    concat(
        trim(e.first_name), ' ', trim(e.last_name)
...
-- Consulta 7: Clientes cuya primera compra
-- fue durante una promoción
-- Encuentra clientes que empezaron comprando
-- productos gratuitos (precio = 0)
